...
void gyro_drive(std::shared_ptr<ChassisController> chassis, QLength distance, double max_speed, bool drive_straight)
{
    ...
    std::valarray<std::int32_t> start_pos_values = get_sensor_vals(chassis);
    //Calculates current position based on start position (found in chassisController.cpp on github)
    std::valarray<std::int32_t> current_pos_values = get_sensor_vals(chassis) - start_pos_values;
    ...
    //while (fabs(last_three_derivatives) > epsilon)
    {
        ...
        chassis_x_model->xArcade(0.0, speed, turn_speed);
        pros::delay(33);
